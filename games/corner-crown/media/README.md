...
| `play.png` | `local-rom-gallery-play` | 6手目まで進めた盤面。黄色の◆が角 |
| `clear.png` | `local-rom-gallery-goal` | 角を4つ取り39対25で勝利 |

`goal.webm`は同じROM/FONT起動から`local-rom-gallery-goal`のreplayを1/30秒ごとに記録した映像と、同じ実行の
PCMから作った動画です（50.6秒、3.0倍速）。画面全体を無加工で収録しました。`tools/capture_video.py`と外部のffmpegで
