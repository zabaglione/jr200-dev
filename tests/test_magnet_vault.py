...
"""MAGNET VAULT: upstream vaults, pulls and ratings on sdk/ranked.inc, replays and sound."""
import json
import sys
...
PROJECT = ROOT / 'games/magnet-vault'
sys.path.insert(0, str(ROOT / 'tests'))
from port_model import check_expectations, load_game_model  # noqa: E402
import ranked_model as rm  # noqa: E402

fs = load_game_model('magnet-vault')


def new_port():
    return rm.RankedPortModel(fs.MagnetVault(), fs.LEVELS, fs.TAG)
...
class MagnetVaultRuleTests(unittest.TestCase):
    ...
    def test_boxes_block_walking_and_are_pulled_one_step(self):
        port = play(0, [])
        g = port.game
        g.b, g.c = [0] * 64, [0] * 64
        g.pos, g.facing, g.c[28] = 27, 3, 1     # a box to the right
        g.act(4)
        self.assertEqual((g.pos, g.facing, g.moves), (27, 4, 1))   # turned only
        g.act(5)
        self.assertEqual((g.pos, g.c[27], g.c[28], g.pulls, g.moves), (26, 1, 0, 1, 2))
        g.b[25] = 1                              # a wall behind: no pull
        g.act(5)
        self.assertEqual((g.pos, g.pulls, g.moves), (26, 1, 2))


class MagnetVaultExpectationTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_all_forty_stages_and_password_are_covered(self):
        end = self.profiles['synthetic-vaults-31-40']['expect']['memory']
