...
"""GLYPH SHIFT: upstream chambers, rule swaps and ratings on sdk/ranked.inc, replays and sound."""
import json
import sys
...
PROJECT = ROOT / 'games/glyph-shift'
sys.path.insert(0, str(ROOT / 'tests'))
from port_model import check_expectations, load_game_model  # noqa: E402
import ranked_model as rm  # noqa: E402

fs = load_game_model('glyph-shift')


def new_port():
    return rm.RankedPortModel(fs.GlyphShift(), fs.LEVELS, fs.TAG)
...
class GlyphShiftRuleTests(unittest.TestCase):
    ...
    def test_the_rule_decides_which_glyph_is_a_wall(self):
        port = play(0, [])
        g = port.game
        g.b = [0] * 64
        g.pos, g.b[g.pos + 1] = 27, 4           # a box to the right
        g.act(4)
        self.assertEqual(g.pos, 27)             # phase 0: boxes are walls
        g.act(5)
        g.act(4)
        self.assertEqual((g.pos, g.phase, g.changes, g.moves), (28, 1, 1, 2))


class GlyphShiftExpectationTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_all_forty_stages_and_password_are_covered(self):
        end = self.profiles['synthetic-glyphs-21-40']['expect']['memory']
